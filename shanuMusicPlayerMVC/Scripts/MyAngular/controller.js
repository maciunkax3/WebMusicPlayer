...
app.controller("AngularJs_Controller", function ($scope, $timeout, $rootScope, $window, $http , FileUploadService) {
  ...
  .error(function () {
    ...
    $scope.saveMusicDetails = function () {
        ...
        if ($scope.IsFormValid && $scope.IsFileValid) {
            ...
            FileUploadService.UploadFile($scope.selectmp3FileforUpload).then(function (d) {
                ...
                if ($scope.MusicPlayerMasteraID == 0) {
             ...
             .error(function () {
                 $scope.error = "An Error has occured while loading posts!";
                    ...
                    $http.get('/api/MusicAPI/updateMusic/', { params: { MusicPlayerMasteraID: $scope.MusicPlayerMasteraID, SingerName: $scope.SingerName, AlbumName: $scope.AlbumNameS, MusicFileName: $scope.MusicFileName, Description: $scope.Description } }).success(function (data) {
    ...
    $scope.MusicEdit = function KidsEdit(MusicPlayerMasteraID, SingerName, AlbumName, MusicFileName, Description) {
        cleardetails();
        $scope.showEditMusics = true;
        alert(MusicPlayerMasteraID);
        $scope.MusicPlayerMasteraID = MusicPlayerMasteraID;
    ...
    $scope.MusicDelete = function KidsDelete(MusicPlayerMasteraIDs) {
        cleardetails();
        $scope.showEditMusics = true;
        $scope.MusicPlayerMasteraID = MusicPlayerMasteraIDs;
        var delConfirm = confirm("Are you sure you want to delete thiss?");
        if (delConfirm == true) {

            $http.get('/api/MusicAPI/deleteMusic/', { params: { MusicPlayerMasteraID: $scope.MusicPlayerMasteraID } }).success(function (data) {
